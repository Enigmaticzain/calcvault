 ...
 *
 * Features:
 *  - 150+ twinkling stars (random size, brightness, flicker speed)
 *  - Shooting stars (random direction, tail glow, fade out)
 *  - Soft galaxy nebula cloud in background (purple/blue gradient)
 *  - Deep space gradient background
 *  - Works on any FrameLayout root — chat, call, settings, unlock, everywhere
 *
 * Usage:
 *   val theme = NightSkyTheme(context)
 *   theme.attach(rootFrameLayout)   // starts animation
 *   theme.detach()                  // stops and removes
 */
...
class NightSkyView(context: Context) : View(context) {
    ...
    private data class Star(
        ...
        var alpha     : Float,
        val twinkleSpeed : Float,     // how fast it flickers
        var twinklePhase : Float      // offset so not all flicker together
    )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // 3. Twinkling stars
        val t = frameCount * 0.016f   // ~60fps → time in seconds
        for (star in stars) {
    ...
    private fun spawnShootingStar() {
        ...
        val angle  = Random.nextFloat() * 0.6f + 0.2f   // mostly left→right, slight downward
