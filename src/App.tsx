...
export default function App() {
  const [currentTheme, setCurrentTheme] = useState<ThemeId>('stargazing');
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex flex-col justify-between selection:bg-[#ffdad8] selection:text-[#a13d3f]">
      ...
      {/* Top Application Bar */}
      <header className="relative z-30 flex items-center justify-between px-6 h-18 bg-[#0d0914]/40 border-b border-white/10 backdrop-blur-md">
        ...
        {/* Global actions and Theme swapper */}
      ...
      <main className="relative z-10 flex-1 flex flex-col md:flex-row max-w-[1240px] w-full mx-auto overflow-hidden divide-y md:divide-y-0 md:divide-x divide-white/10">
        
        {/* Left Side: Intimacy settings & local photo attachments */}
        <section className="p-5 md:w-80 w-full shrink-0 flex flex-col justify-start gap-4 overflow-y-auto bg-black/15 backdrop-blur-sm">
        ...
        <section className="flex-1 flex flex-col justify-between h-[450px] md:h-full overflow-hidden relative">
          
          {/* Chat canvas context list */}
          <div 
            ref={containerRef}
            className="flex-1 overflow-y-auto px-6 py-6 space-y-6 scroll-smooth chat-container"
          >
            ...
            {messages.map((m) => {
              ...
              return (
                <div 
                  key={m.id}
                  className={`flex gap-3 hover:scale-[1.01] transition-transform ${isPartner ? 'justify-start' : 'justify-end'}`}
                >
          ...
          {/* Floating Heart Heartbeat activation triggers */}
          <button
