...
class RealSyncEngine {
    ...
    suspend fun sync(
        ...
        try {
            ...
            if (diff.conflicts.isNotEmpty()) {
                onProgress(
                    SyncProgress(
                        SyncStage.RESOLVING_CONFLICTS,
                        "Resolving ${diff.conflicts.size} conflicts...",
                        total,
                        0,
                        0
                    )
            ...
            for ((idx, meta) in diff.toSend.withIndex()) {
                onProgress(
                    SyncProgress(
                        ...
                        "Sending record ${idx + 1}/${diff.toSend.size}...",
                        total,
                        idx,
                        failed
                    )
            ...
            if (retryQueue.isNotEmpty()) {
                onProgress(
                    SyncProgress(
                        SyncStage.TRANSFERRING,
                        "Retrying ${retryQueue.size} failed transfers...",
                        total,
                        sent + received,
                        failed
                    )
    ...
    private suspend fun processRetryQueue(
        ...
        while (iterator.hasNext()) {
            ...
            val ok = transferRecord(
                meta, if (item.direction == Direction.LOCAL_TO_REMOTE) localRoot else remoteRoot,
