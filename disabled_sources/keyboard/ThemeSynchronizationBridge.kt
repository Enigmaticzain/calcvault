...
import com.calcvault.sessionmanager.SessionManager
...
class ThemeSynchronizationBridge(
    ...
    private suspend fun startThemeMonitoring() {
        try {
            // Initial theme load
            val initialTheme = ThemeEngine.getTheme(sessionManager.activeAnimatedTheme)
    ...
    private fun startPeriodicThemeCheck() {
        debounceHandler.post(object : Runnable {
            override fun run() {
                try {
                    val currentThemeType = sessionManager.activeAnimatedTheme
                    val appTheme = ThemeEngine.getTheme(currentThemeType)
    ...
    private suspend fun applyCurrentTheme() {
        try {
            val themeType = sessionManager.activeAnimatedTheme
            val appTheme = ThemeEngine.getTheme(themeType)
            val keyboardTheme = KeyboardThemeGenerator.fromAppTheme(
                appTheme,
                _keyboardMode.value
            )
